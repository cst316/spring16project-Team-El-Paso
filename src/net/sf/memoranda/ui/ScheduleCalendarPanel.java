...
public class ScheduleCalendarPanel extends JPanel {
  ...
  ScheduleCalendar schCalendar = new ScheduleCalendar(CurrentDate.get());
  JPanel schCalendarPanel = new JPanel();
  BorderLayout borderLayout5 = new BorderLayout();
  JSpinner yearSpin = new JSpinner(new SpinnerNumberModel(schCalendar.get().getYear(), 1980, 2999, 1));
  ...
  void jbInit() throws Exception {
    ...
    schCalendar.getTableHeader().setFont(new java.awt.Font("Dialog", 1, 10));
    schCalendar.setFont(new java.awt.Font("Dialog", 0, 10));
    schCalendar.setGridColor(Color.lightGray);
    schCalendarPanel.setLayout(borderLayout5);
    ...
    this.add(schCalendarPanel,  BorderLayout.CENTER);
    schCalendar.getTableHeader().setPreferredSize(new Dimension(200, 15));
    schCalendarPanel.add(schCalendar.getTableHeader(), BorderLayout.NORTH);
    schCalendarPanel.add(schCalendar, BorderLayout.CENTER);
    schCalendar.addSelectionListener(new ActionListener()  {
      public void actionPerformed(ActionEvent e) {
        setCurrentDateDay(schCalendar.get(), schCalendar.get().getDay());
      }
    });
